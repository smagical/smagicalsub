...
export async function loadSubscriptionHealth(token: string, format: TokenSubscriptionFormat, customPath?: string | null): Promise<SubscriptionHealthResult> {
  try {
    ...
    if (format !== "sing-box" && format !== "xray") {
      return { detail: "订阅内容正常", ok: true, statusText: "HTTP 200" };
    }

    // JSON 配置会被客户端直接解析，这里提前捕获格式错误。
    try {
      JSON.parse(trimmed);
    } catch {
      return { detail: `${format} 订阅不是合法 JSON`, ok: false, statusText: "JSON 格式错误" };
    }

    return { detail: `${format} JSON 格式正常`, ok: true, statusText: "HTTP 200" };
  } catch (error) {
    return {
