...
export function TokenOutputDiagnosticsPanel({ diagnostics }: TokenOutputDiagnosticsPanelProps) {
  return (
    <div className="mt-3 grid gap-3 rounded-md border bg-background/70 p-3 shadow-sm ring-1 ring-border/40 md:grid-cols-[1fr_1fr]">
      ...
      <div className="flex flex-col justify-between gap-2 text-sm">
        <span className="rounded-md bg-muted/50 px-3 py-2 text-muted-foreground">手动节点 {diagnostics.manualNodeCount} 个，订阅源节点 {diagnostics.sourceNodeCount} 个。</span>
        ...
        {diagnostics.warnings.length > 0 ? <p className="rounded-md border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">{diagnostics.warnings.join("；")}</p> : null}
      </div>
    </div>
  );
}
