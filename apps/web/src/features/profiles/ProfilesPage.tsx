...
import { CheckCircle2, Clock3, FileSliders, ListFilter, Route, SlidersHorizontal } from "lucide-react";
...
import { ProfileModulesPanel } from "./ProfileModulesPanel";
...
export function ProfilesPage() {
  ...
  return (
    <ModulePanel eyebrow="Profiles" title="配置档" description="维护订阅配置档，后续规则和令牌绑定会基于这里扩展。" tone="green">
      ...
      <ProfileTools
        mergePreview={page.mergePreview}
        pending={page.pending}
        profiles={page.profiles}
        onCreateFromPreview={page.createProfileFromPreview}
        onPreviewMerge={page.previewProfileMerge}
      />

      <ProfileModulesPanel
        modules={page.profileModules}
        pending={page.pending}
        profiles={page.profiles}
        onCreate={page.createProfileModule}
        onDelete={page.deleteProfileModule}
        onUpdate={page.updateProfileModule}
      />
          ...
          {page.filteredProfiles.length === 0 ? (
            ...
            <>
              ...
              <ListPagination
                ...
                total={page.filteredProfiles.length}
              />
            </>
          )}
        </div>
...
function ProfileOverview({
  ...
  return (
    <section className="grid gap-3 rounded-xl border bg-card p-3 xl:grid-cols-[minmax(300px,0.8fr)_minmax(0,1.2fr)]">
      <div className="grid gap-2.5 rounded-lg border bg-background/70 p-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <h3 className="truncate text-lg font-semibold leading-tight">配置档与规则编排</h3>
          </div>
        ...
        <p className="text-xs leading-5 text-muted-foreground">把默认策略、规则排序和订阅令牌绑定集中维护，输出时会按这里的规则生成配置。</p>
      </div>
